   ...
   IN  DIGIT syndrome[NUM_DIGITS_GF2X_ELEMENT])
{
   ...
   do {
      ...
      iter++;
      // printf("~");
   } while ((iter < 0.8 * NUM_ERRORS_T) && (hw != 0));
   
   //
   int flip_ups[V+1] = {0};
   int flip_downs[V+1] = {0};
   int cur_weight = population_count(error) + population_count(error + NUM_DIGITS_GF2X_ELEMENT);
   // scan all bits of error
   for (int block = 0; block < N0; block++) {
      for (int bit = 0; bit < P; bit++) {
         if (upc[block * P + bit] >= (V + 1) / 2) {
            if (get_coeff(error + block * NUM_DIGITS_GF2X_ELEMENT, bit)) {
               flip_downs[upc[block * P + bit]] += 1;
            }
            else {
               flip_ups[upc[block * P + bit]] += 1;
            }
         }
      }
   }
   // compute cumulative distribution for flip_ups and flip_downs in place
   for (int i = V-1; i >= 0; i--) {
      flip_ups[i] += flip_ups[i+1];
      flip_downs[i] += flip_downs[i+1];
   }

   int chosen_th = (V+1)/2;
   for (int i = (V+1)/2; i <= V; i++) {
      if(flip_ups[i] - flip_downs[i] + cur_weight == NUM_ERRORS_T) {
         chosen_th = i;
         // printf("Chosen threshold: %d / %d\n", chosen_th, (V+1)/2);
         break;
      }
   }
